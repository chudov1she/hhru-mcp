"""
MCP-сервер "hh-connector" для работы с hh.ru от имени работодателя.

Транспорт: stdio (для Claude Desktop / Claude Code / Hermes / Cursor и др.).

Tools:
  - whoami                  : кто авторизован (работодатель)
  - list_vacancies          : список активных вакансий работодателя
  - list_negotiations       : отклики на вакансию (по этапу)
  - get_negotiation         : карточка отклика
  - get_resume              : полное резюме кандидата
  - get_messages            : переписка по отклику (чат)
  - list_allowed_actions    : допустимые этапы/действия по отклику
  - send_message_to_candidate : ОТПРАВКА РЕАЛЬНОГО СООБЩЕНИЯ кандидату от имени
                                работодателя (переводит отклик на этап). Использовать
                                только по явной просьбе пользователя!
  - get_token_status        : статус токена
  - refresh_token           : принудительный refresh access-токена

Запуск (stdio):
  python mcp_server.py

Запуск (SSE/HTTP, если MCP-клиент требует): смотри README.
"""
